...
// Return Value:
// - <none>
void IslandWindow::UnregisterAllHotKeys()
{
    TraceLoggingWrite(
        g_hWindowsTerminalProvider,
        "UnregisterAllHotKeys",
        TraceLoggingDescription("Emitted when clearing previously set hotkeys"),
        TraceLoggingInt32(_nextHotkeyIndex, "numHotkeys", "The number of hotkeys to unset"),
        TraceLoggingLevel(WINEVENT_LEVEL_VERBOSE),
        TraceLoggingKeyword(TIL_KEYWORD_TRACE));

    for (; _nextHotkeyIndex != 0; --_nextHotkeyIndex)
    {
        LOG_IF_WIN32_BOOL_FALSE(::UnregisterHotKey(_window.get(), _nextHotkeyIndex));
    }
}
...
// - Call UnregisterAllHotKeys() before registering your hotkeys.
...
// Return Value:
// - <none>
void IslandWindow::RegisterHotKey(const winrt::Microsoft::Terminal::Control::KeyChord& hotkey)
{
    TraceLoggingWrite(
        g_hWindowsTerminalProvider,
        "RegisterHotKey",
    ...
    LOG_IF_WIN32_BOOL_FALSE(::RegisterHotKey(_window.get(), _nextHotkeyIndex, hotkeyFlags, vkey));
    ++_nextHotkeyIndex;
}
